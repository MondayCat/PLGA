...
class FederateDataSplit:
    ...
    def get_split_result(self,**kwargs):
        """
        ...
        if kwargs["dataset_name"] == "CIFAR100":
            org_data = []
            org_targets = []
            for i in range(5):
                for data, targets in data_loader:
                    org_data_tmp = data.clone().detach()
                    org_targets_tmp = targets.clone().detach()
                    org_data.append(org_data_tmp)
                    org_targets.append(org_targets_tmp)
            org_data = torch.cat(org_data,dim=0)
            org_targets = torch.cat(org_targets, dim=0)
            all_data_num = all_data_num*5
        else:
            for data, targets in data_loader:
        ...
        # print(org_data.shape, org_targets.shape)
        # exit()
        #  get per class index
        class2index = {}
        ...
        for i in range(self.num_worker):
            ...
            for cur_index_list in cur_select_d_index_lists:
                self.array = np.array(org_targets[cur_index_list].cpu())
